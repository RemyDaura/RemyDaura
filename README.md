# Salut, je suis Rémy ! 👋

### Étudiant Data Analyst @ OpenClassrooms
**Actuellement à la recherche d'un stage de 2 à 6 mois (Démarrage : Dès maintenant)**

Passionné par les chiffres qui parlent, je transforme la donnée brute en décisions stratégiques. 
Mon parcours chez OpenClassrooms m'a permis de maîtriser la chaîne de valeur de la donnée, de l'extraction à la visualisation.

---

### Ma "Data Stack"
Voici les outils que j'utilise au quotidien pour résoudre des problèmes business :

| Domaine | Technologies & Outils |
| :--- | :--- |
| **Analyse & Scripting** | Python (Pandas, NumPy, Scikit-learn), R |
| **Bases de Données** | SQL (MySQL, PostgreSQL), Modélisation (MCD/MLD) |
| **Visualisation (Dataviz)** | Tableau, PowerBI, Matplotlib, Seaborn |
| **Outils** | Jupyter Notebooks, Excel (Avancé), Git/GitHub |

---

### Mes Projets Phares
J'aime apprendre en faisant. Voici quelques projets concrets réalisés durant ma formation :

* **[Analyse de ventes E-commerce]https://github.com/RemyDaura/DA-Projet2-Analyse-Ventes-Excel**
    * *Nettoyage de données et création de tableaux de bord dynamiques pour piloter la performance commerciale.*
    * `Excel` `KPIs` `Business Analysis`

* **[Étude de Santé Publique]https://github.com/RemyDaura/DA-Projet4-Etude-Sante-Python**
    * *Exploration de données mondiales (FAO) pour détecter des tendances de malnutrition et proposer des solutions.*
    * `Python` `Data Cleaning` `Pandas`

* **[Architecture BDD Immobilière] https://github.com/RemyDaura/DA-Projet5-Immobilier-SQL**
    * *Conception et implémentation d'une base de données complète pour une agence immobilière.*
    * `SQL` `Modélisation` `SGBD`

---

### Me contacter
Je suis toujours ravi d'échanger sur la Data, ou pour discuter d'opportunités de stage.

 **LinkedIn :** https://www.linkedin.com/in/r%C3%A9my-d-aura/
 **Email :** remy.daura.pro@gmail.com

---
*Merci de ta visite ! N'hésite pas à jeter un œil à mes repositories pour voir mon code.*
